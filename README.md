...
## Verifying a release (Sigstore / cosign)

Every tagged release of gv-llm-pricing includes a Sigstore keyless signature
over `v1/pricing.json`, generated by the `sign.yml` workflow running under
this repo's GitHub Actions OIDC identity. Consumers that want cryptographic
assurance a given `pricing.json` was produced by this repo (and not injected
by a compromised CDN, mirror, or middlebox) can verify it with
...
Download all three artifacts from the GitHub Release page for the tag you
care about — `pricing.json`, `pricing.json.sig`, `pricing.json.crt` — then:

```bash
cosign verify-blob \
  --certificate pricing.json.crt \
  --signature pricing.json.sig \
  --certificate-identity-regexp 'https://github.com/RelayOne/gv-llm-pricing/.github/workflows/sign\.yml@.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  pricing.json
...
signature, a rewritten `pricing.json`, or a certificate issued to a
different workflow — fails loudly.

`cmd/gen-pricing` in `router-core` runs exactly this verification
automatically before embedding a pricing snapshot. See that repo's
`docs/pricing.md` for the consumer-side flow.
